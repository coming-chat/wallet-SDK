...
func TestToken_BuildTransferTransaction(t *testing.T) {
	chain := TestnetChain()
	token := NewTokenMain(chain)

	sender := M1Account(t)
	recipient := M1Account(t).Address()

	coins, err := token.getCoins(sender.Address(), 2)
	require.Nil(t, err)
	require.GreaterOrEqual(t, len(coins), 2)

	balance1 := coins[0].Balance.Uint64()
	balance2 := coins[1].Balance.Uint64()
	...
	tests := []struct {
		...
		// {
		// 	name:       "transfer object 1",
		// 	amount:     strconv.FormatUint(balance1, 64),
		// 	runSuccess: true,
		// },
		{
			name:       "transfer object 2",
			amount:     strconv.FormatUint(balance2, 10),
			runSuccess: true,
		},
		{
			name:       "transfer 1",
			amount:     strconv.FormatUint(balance1-100, 10),
			runSuccess: true,
		},
		{
			name:       "transfer 2",
		...
		},
		{
			...
			amount:  strconv.FormatUint(1e9*100000, 10),
			wantErr: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := token.BuildTransferTransaction(sender, recipient, tt.amount)
			if tt.wantErr {
				require.Error(t, err)
				return
			} else {
			...
			require.Nil(t, err)
			resp, err := cli.DryRunTransaction(context.Background(), &got.Txn)
			require.Nil(t, err)
			require.Equal(t, resp.Effects.Data.IsSuccess(), tt.runSuccess)
		})
	}
}

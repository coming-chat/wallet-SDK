...
func serialize_u16(buf []byte, n uint16) {
	binary.BigEndian.AppendUint16(buf, n)
}

func serialize_u32(buf []byte, n uint32) {
	binary.BigEndian.AppendUint32(buf, n)
}

func serialize_u64(buf []byte, n uint64) {
	binary.BigEndian.AppendUint64(buf, n)
}
...
func serialize_vector_with_length(buf, bytes []byte) {
	if len(bytes) == 0 {
		return
	}

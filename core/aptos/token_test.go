...
func TestTokenBalance(t *testing.T) {
	...
	btcToken, err := NewToken(chain, "0x43417434fd869edee76cca2a4d2301e528a1551b1d719b75c350c3c97d15b8b9::coins::BTC")
	assert.Nil(t, err)
	btcBalance, err := btcToken.BalanceOfAddress(address)
	assert.Nil(t, err)
	t.Logf("BTC Balance = %v", btcBalance.Total)

	usdtToken, err := NewToken(chain, "0x43417434fd869edee76cca2a4d2301e528a1551b1d719b75c350c3c97d15b8b9::coins::USDT")
	assert.Nil(t, err)
...
func TestTokenRegister(t *testing.T) {
	chain := NewChainWithRestUrl(testnetRestUrl)
	tokenTag := "0x43417434fd869edee76cca2a4d2301e528a1551b1d719b75c350c3c97d15b8b9::coins::BTC"
	account, err := NewAccountWithMnemonic(testcase.M1)
	assert.Nil(t, err)

	token, err := NewToken(chain, tokenTag)
	assert.Nil(t, err)
